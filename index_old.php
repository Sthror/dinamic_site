...
*/
    ...
    $a = 5; // сторона a - вершина
    $b = 7;  // сторона b - онсование
    $c = 3;  // сторона c  - боковая
    $d = 3; // сторона d   - боковая 
    $h = 3; // сторона d   - боковая 

    $hAlt =  $c ** 2 - ((($b - $a) ** 2 + $c ** 2 - $d ** 2) / 2 * ($b - $a)) ** 2;
    $hAlt = sqrt($hAlt); // высота
    $control = true;
    if($h !== $hAlt){
        $control = false;
    }
    if ($Why == true && $control) {
        $S = (($a + $b)/2) * $h; // высчитываю площадь
        echo "Площадь трапеции равна $S"; // вывожу результат

    } else if ($Why == false && $control){
        $perimetr =  $a + $b + $c + $d; // высчитываю периметр      
        echo "<p>Периметр равен $perimetr !</p>"; // вывожу результат
    }
...
*/
    ...
    for ($i = 0; $i < 100; $i++) { // счетчик 100 итераций
...
*/
    ...
    $key = (($pass1 % 100) - ($pass1 % 10)) / 10; // второй разряд pass1
    $razryad_1 = $pass2Se % 10; // первый разряд
    $razryad_2 = (($pass2Se - $razryad_1) / 10)%10; // второй разряд
...
*/
    ...
    $razryad_2 = (($pass2 - $razryad_1) / 10)%10; // второй разряд
...
*/
    ...
    if (substr($IP, 3, 1) !== '.') {
        $IP4 = true;
        $Y4 = (int)($IP/(256**3));       
        $Y3 = (int)(($IP - $Y4*(256**3))/256**2);
        $Y2=(int)(($IP - $Y4*(256**3)-$Y3*(256**2))/256);
        $Y1=$IP-$Y4*(256**3)-$Y3*(256**2)-$Y2*256;
        $decodeIP = "$Y4.$Y3.$Y2.$Y1";
    }
    while ($itaration && !$IP4) {
        if ($x == 3) {
            $multi = 256**3;
